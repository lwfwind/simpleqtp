            ...
            <div id="Summary" Style="position:relative; display:block; text-align:center">
                <table id="TableSummary" cellSpacing="0" cellPadding="0">
                    ...
                    <tr class = "Count">
                        ...
                        <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0])"/></td>
                        <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0)])" /></td>
                    </tr>
                    </xsl:for-each>
                    <tr class = "Count">
                        <td>Total</td>
                        <td><xsl:value-of select="count(Report/TestSuite/TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0]) "/></td>
                        <td><xsl:value-of select="count(Report/TestSuite/TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0)]) "/></td>
                    </tr>
                </table>
            </div>
            <br/>
                ...
                <div id="SummaryOfTestCase" Style="position:relative; display:none; text-align:center">
                    <table id="SummaryOfTestCaseTable" cellSpacing="0" cellPadding="0">
                        ...
                        <tr class = "TotalPassed">
        					<td>Total Passed</td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0 and contains(translate(@ID,$lower,$upper), '_C_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0 and contains(translate(@ID,$lower,$upper), '_H_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0 and contains(translate(@ID,$lower,$upper), '_M_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0 and contains(translate(@ID,$lower,$upper), '_L_') ])"/></td>
                        </tr>

                        <tr class = "TotalFailed">
                            <td>Total Failed</td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0) and contains(translate(@ID,$lower,$upper), '_C_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0) and contains(translate(@ID,$lower,$upper), '_H_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0) and contains(translate(@ID,$lower,$upper), '_M_') ])"/></td>
                            <td><xsl:value-of select="count(TestCase[count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0) and contains(translate(@ID,$lower,$upper), '_L_') ])"/></td>
                        </tr>
        			</table>
        		</div>
            <br/>
			...
			<table cellSpacing="1" cellPadding="1"  onClick="toggleMenu('div{position()}.0', 'm{position()}.0')">
				<tr>
                    ...
                    <td class="Passed">Passed -
                        <xsl:value-of select ="count(TestCase[count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0])"/>
                    </td>
					<td class="Failed">Failed -
						<xsl:value-of select ="count(TestCase[count(Step[@Status='2']) > 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0)])"/>
                    </td>
                </tr>
			</table>
		</div>
    					...
    					<td class="message">
    					 ...
    					 <xsl:element name="td">
                              <xsl:attribute name="class">
                                <xsl:if test="count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0">
                                Passed
                                </xsl:if>
                                <xsl:if test="count(Step[@Status='2']) &gt; 0 or (count(Step[@Status='2']) = 0 and count(Step[@Status='1']) = 0)">
                                Failed
                                </xsl:if>
                			</xsl:attribute>
             			    <xsl:if test="count(Step[@Status='2']) = 0 and count(Step[@Status='1']) != 0">
